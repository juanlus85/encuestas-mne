...
function EncuestadorHome() {
  ...
  // GPS for quick rejections (continuously updated)
  ...
  const addRejection = trpc.rejections.add.useMutation({
    onSuccess: (_, vars) => {
      toast.success(`${vars.surveyType === "residentes" ? "Resident" : "Visitor"} rejection recorded`, { duration: 1500 });
    },
    onError: (err) => toast.error("Failed to record rejection: " + err.message),
  });
  ...
  const greeting = hour < 12 ? "Good morning" : hour < 20 ? "Good afternoon" : "Good evening";
...
Survexia field operations
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 md:px-8 space-y-6">
        <div className="grid grid-cols-2 gap-3">
          <Card className="border-0 shadow-sm">
            <CardContent className="p-4 text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1">Surveys today</p>
            </CardContent>
          </Card>
          <Card className="border-0 shadow-sm">
            <CardContent className="p-4 text-center">
              <p className="text-3xl font-bold text-primary">{myResponses?.length ?? 0}</p>
              <p className="text-sm text-muted-foreground mt-1">Completed total</p>
            </CardContent>
          </Card>
        </div>

        <div>
          <h2 className="text-base font-semibold text-foreground mb-3">Pedestrian Counting</h2>
          <button
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-foreground">Pedestrian Counting</p>
                <p className="text-sm text-muted-foreground">Record groups of people by direction</p>
              </div>
            </div>
          </button>
        </div>
        ...
        <div>
          <h2 className="text-base font-semibold text-foreground mb-3">Active Surveys</h2>
          ...
          <div className="space-y-3">
            {templates?.map((t) => (
              <div key={t.id} className="flex items-stretch gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                          ...
                          {t.type === "residentes" ? "Residents" : "Visitors"}
                        </span>
                      </div>
                      <p className="font-semibold text-foreground">{t.nameEn || t.name}</p>
                      {(t.descriptionEn || t.description) && (
                        <p className="text-sm text-muted-foreground mt-0.5 line-clamp-2">{t.descriptionEn || t.description}</p>
                      )}
                    </div>
                ...
                <button
                  ...
                  title={`Record ${t.type === "residentes" ? "resident" : "visitor"} rejection`}
                ...
                >
                  ...
                  <span className="text-[10px] font-semibold leading-tight text-center">
                    Reject<br />{t.type === "residentes" ? "res." : "vis."}
                  </span>
                </button>
              </div>
            ))}
            {(!templates || templates.length === 0) && (
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-sm">There are no active surveys right now.</p>
              </div>
            )}
          </div>
        </div>
        ...
        {myResponses && myResponses.length > 0 && (
          <div>
            <h2 className="text-base font-semibold text-foreground mb-3">Latest Surveys</h2>
            <div className="space-y-2">
              {myResponses.slice(0, 5).map((r) => (
                <div key={r.id} className="flex items-center gap-3 bg-card border border-border rounded-lg px-4 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate">Survey #{r.id}</p>
...
Survexia research operations dashboard
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <Card className="border-0 shadow-sm">
            <CardContent className="p-5">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Total Surveys</p>
                  <p className="text-3xl font-bold text-foreground mt-1">{total}</p>
                  <p className="text-xs text-muted-foreground mt-1">of {TARGET_TOTAL} target</p>
                </div>
              ...
              <p className="text-xs text-muted-foreground mt-1">{pctTotal}% completed</p>
            </CardContent>
          </Card>
          ...
          <Card className="border-0 shadow-sm">
            <CardContent className="p-5">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Residents</p>
                  <p className="text-3xl font-bold text-blue-600 mt-1">{residentes}</p>
                  <p className="text-xs text-muted-foreground mt-1">of {TARGET_RESIDENTES} target</p>
                </div>
              ...
              <p className="text-xs text-muted-foreground mt-1">{pctResidentes}% completed</p>
            </CardContent>
          </Card>
          ...
          <Card className="border-0 shadow-sm">
            <CardContent className="p-5">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Visitors</p>
                  <p className="text-3xl font-bold text-amber-600 mt-1">{visitantes}</p>
                  <p className="text-xs text-muted-foreground mt-1">of {TARGET_VISITANTES} target</p>
                </div>
              ...
              <p className="text-xs text-muted-foreground mt-1">{pctVisitantes}% completed</p>
            </CardContent>
          </Card>
          ...
          <Card className="border-0 shadow-sm">
            <CardContent className="p-5">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Interviewers</p>
                  <p className="text-3xl font-bold text-foreground mt-1">{encuestadores?.length ?? 0}</p>
                  <p className="text-xs text-muted-foreground mt-1">active in the field</p>
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          >
            ...
            <p className="text-sm text-muted-foreground mt-1">Charts and analysis of fieldwork activity</p>
          </button>
          ...
          >
            ...
            <p className="font-semibold text-foreground">Field Map</p>
            <p className="text-sm text-muted-foreground mt-1">GPS locations and heatmap</p>
          </button>
          ...
          >
            ...
            <p className="text-sm text-muted-foreground mt-1">Full survey list</p>
          </button>
        </div>
